          ...
          {filteredItems.map((item) => (
            <motion.div
              ...
              whileHover={{ 
                ...
                boxShadow: "0 25px 45px rgba(0,0,0,0.6), 0 0 30px rgba(255,107,0,0.2)"
              }}
              transition={{ type: "spring", stiffness: 300, damping: 18 }}
              style={{ transformStyle: "preserve-3d" }}
              onClick={() => setActiveVideoItem(item)}
              className="group relative rounded-xl overflow-hidden bg-neutral-950 border border-white/5 hover:border-brand-orange/45 transition-all cursor-pointer aspect-[4/3] flex flex-col justify-end"
            >
              {/* Image backdrop */}
